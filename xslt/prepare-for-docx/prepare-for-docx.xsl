...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:epub="http://www.idpf.org/2007/ops" xmlns:f="#"
   ...
   <xsl:template
      ...
      <!-- no extra p before if preceding-sibling[last()] is: page-nr or after another hx, or if hx
      contains pagenr -->
      <xsl:if
         test="not(preceding-sibling::*[position() = last()][name() ='div' or name() = 'span'][@class='page-normal'] or preceding-sibling::*[1][matches(name(), '^h[1-6]$')] or ((count(preceding-sibling::*)=0 and ../preceding-sibling::*[1][matches(name(), '^h[1-6]$')])) or child::span[f:types(.) = 'pagebreak'])">
         <!-- <xsl:if test="not(preceding-sibling::*[position() = last()][name() ='div' or name() =
         'span'][@class='page-normal'] or preceding-sibling::*[1][matches(name(), '^h[1-6]$')] or
         ((count(preceding-sibling::*)=0 and ../preceding-sibling::*[1][matches(name(),
         '^h[1-6]$')])))"> -->
         <p />
      </xsl:if>
      <xsl:copy
         ...
         <xsl:apply-templates select="@*" />
         <xsl:text>xxx</xsl:text><xsl:value-of
            select="substring(name(),2,1)" /><xsl:text> </xsl:text>
         <xsl:apply-templates
            select="node()" />
      </xsl:copy>
   </xsl:template>
   ...
   <xsl:function name="f:movePageTopLevel">
      <xsl:param name="element" as="element()" />
        <xsl:variable name="movePageTopLevel"
         as="xs:boolean"
         select="not($element/ancestor::*[matches(name(), 'h[1-6]|table|th|td|p|li') or $element/ancestor::section[f:types(.)='toc']/ol[@class='list-type-none']])" />
        <xsl:value-of
         select="$movePageTopLevel" />
   </xsl:function>
   <xsl:function name="f:movePageBefore">
      <xsl:param name="element" as="element()" />
        <xsl:variable name="movePageBefore" as="xs:boolean"
         select="matches($element/name(), 'h[1-6]|table|th|td') or exists($element/ancestor::section[f:types(.)='toc']/ol[@class='list-type-none'])" />
        <xsl:value-of
         select="$movePageBefore" />
   </xsl:function>
   <!--<xsl:template
   match="*[f:movePageBefore(.)=true() and (.//span[@epub:type = 'pagebreak'] or .//div[@epub:type =
   'pagebreak']) and f:movePageTopLevel(.)=true()]" priority="2"> removed on 08.12.23 solve the
   issue of span in p  -->
   ...
   <!-- Replaced with the code above, 2024-02-14. Kvile and Stephen -->
   <!-- <xsl:template match="div[f:types(.) = 'pagebreak' and f:movePageBefore(parent::*)=false()
   and f:movePageTopLevel(parent::*)=true() and not(ancestor::li)] | span[f:types(.) = 'pagebreak'
   and f:movePageBefore(parent::*)=false() and f:movePageTopLevel(parent::*)=true() and
   not(ancestor::li)]"> -->
   <xsl:template
      ...
      <!-- Replaced with the code above, 2024-02-14. Kvile and Stephen -->
      <!-- <xsl:template match="div[f:types(.) = 'pagebreak' and f:movePageBefore(parent::*)=false()
      and f:movePageTopLevel(parent::*)=true() and not(ancestor::li)] | span[f:types(.) =
      'pagebreak' and f:movePageBefore(parent::*)=false() and f:movePageTopLevel(parent::*)=true()
      and not(ancestor::li)]"> -->
      <!-- <xsl:template match="(div | span)[f:types(.) = 'pagebreak' and
      f:movePageBefore(parent::*)=false() and f:movePageTopLevel(parent::*)=true() and
      not(ancestor::li)]"> -->
        <xsl:call-template
         name="create-pagebreak">
         <xsl:with-param name="element" select="." />
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="p[f:movePageTopLevel(.)=true()]" priority="2">
      ...
      <xsl:next-match />
      <xsl:for-each
         select="descendant::span[f:types(.) = 'pagebreak']">
         <xsl:call-template name="create-pagebreak">
            <xsl:with-param name="element" select="." />
         </xsl:call-template>
      </xsl:for-each>
   ...
   <xsl:template
      match="figcaption/p[f:movePageBefore(.)=false()] | figure[f:classes(.) = 'image']/aside/p[f:movePageBefore(.)=false()] | caption/p[f:movePageBefore(.)=false()]"
      priority="3">
      <xsl:apply-templates />
      <xsl:if
         test="position() != last() and not(exists(following-sibling::*[1][name() = 'ol' or name() = 'ul' or name() = 'table']))">
         <br />
      </xsl:if>
   ...
   <xsl:template
      match="li[f:movePageBefore(.)=false() and .//span[@epub:type = 'pagebreak'] and not(exists(.//li//span[@epub:type = 'pagebreak']))]">
      <xsl:copy>
   ...
   <xsl:template match="li[ancestor::section[f:types(.) = 'toc']]">
      <xsl:copy exclude-result-prefixes="#all">
         <xsl:apply-templates select="@*" />
         <xsl:value-of
            select="concat('xxx', count(ancestor-or-self::li), ' ')" />
         <xsl:apply-templates
            select="node()" />
      </xsl:copy>
   </xsl:template>
   ...
   <xsl:template match="body">
      <xsl:copy exclude-result-prefixes="#all">
         ...
         <xsl:variable
            name="publisher" select="/*/head/meta[@name = 'dc:publisher']/@content" />
         <xsl:variable
            name="publisher-location"
            select="/*/head/meta[@name = 'dc:publisher.location']/@content" />
         <xsl:variable
            name="issued" select="/*/head/meta[@name = 'dc:date.issued']/@content" />
         <xsl:variable
            name="issued-original" select="/*/head/meta[@name = 'dc:issued.original']/@content" />
         <xsl:variable
            name="edition-original"
            select="/*/head/meta[@name = 'schema:bookEdition.original']/@content" />
         <xsl:variable
            name="edition-original" select="replace($edition-original, '^(\d+\.?)$', '$1.utg.')" />
         <!-- Replace "1" with "1.utg." -->
         <xsl:variable
            name="edition-original" select="replace($edition-original, '\.+', '.')" />
         <!-- Replace "1..utg." with "1.utg." -->
         <xsl:variable
            name="pagebreaks" select="(//div | //span)[f:types(.) = 'pagebreak']" />
         <xsl:variable
            name="first-page"
            select="
         ...
         <xsl:variable
            name="isbn" select="/*/head/meta[@name = 'schema:isbn']/@content" />
         <p>
            <xsl:attribute name="xml:lang">no</xsl:attribute>
            <xsl:attribute name="lang">no</xsl:attribute>
            ...
            <xsl:choose>
               <xsl:when test="count($language) gt 1">
                  <xsl:text> - </xsl:text>
                  <xsl:value-of
                     select="string-join($language[position() lt last()], ', ')" />
                  <xsl:text>/</xsl:text>
                  <xsl:value-of
            ...
            <xsl:choose>
               <xsl:when test="count($authors) gt 1">
                  <xsl:text> - </xsl:text>
                  <xsl:value-of
                     select="string-join($authors[position() lt last()], ', ')" />
         ...
         <div>
            ...
            <p>
               <span xml:lang="no" lang="no">xxx innleder overskrifter. Overskriftsnivået vises med
      tall: xxx1, xxx2 osv.</span>
            </p>
            <p>
            ...
            <p>
               <span xml:lang="no" lang="no">Ordforklaringer, gloser eller stikkord finner du etter
      hovedteksten og eventuelle bilder.</span>
            </p>
            <p>
               <span xml:lang="no" lang="no">Eventuelle stikkordsregistre og kilder er utelatt.
      Kolofonen og baksideteksten finner du til slutt i denne filen.</span>
            </p>
         </div>
         <!--   <xsl:apply-templates select="* except section[f:types(.) = ('toc', 'backmatter',
         'index', 'titlepage', 'colophon') and f:classes(.) = ('rearcover')] "/>-->
         <!--  <xsl:apply-templates select="* except section[f:types(.) = ('toc', 'backmatter',
         'index', 'colophon', 'titlepage', 'cover')]"/> -->
         <!--03.05
      ...
      Baksidetekst:</p>
         <xsl:apply-templates
            select="section[f:types(.) = 'cover']/section[f:classes(.) = 'rearcover']" />
         <p />
   ...
   <!-- xsl:template match="ol[following-sibling::*][1][name() = 'ol' or name() = 'ul'] |
   ul[following-sibling::*][1][name() = 'ol' or name() = 'ul']" -->
   <xsl:template
   ...
   <xsl:template match="ul[f:classes(.) = 'list-unstyled']" priority="2">
      ...
      <xsl:if
         test="not(exists(ancestor::table | ancestor::ul | ancestor::ol))">
         <p />
      </xsl:if>
   </xsl:template>
