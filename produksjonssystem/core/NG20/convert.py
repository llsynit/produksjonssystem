...
def convert_ng2020(source: str, target: str, fix_heading_levels: bool, add_header_element: bool) -> None:
    success = True
    ...
    if os.path.exists(target) and not os.path.isdir(target):
        logging.error(
            f"Output '{target}' already exists and is not a directory")
        return False
    if not success:
    ...
    epub_dir: str = epub_dir_obj if isinstance(
        epub_dir_obj, str) else epub_dir_obj.name
    ...
    for root, _, files in os.walk(epub_dir):
        for file in files:
            ...
            if relpath in [item["href"] for item in spine]:
                # skip spine items (we are replacing them with a single HTML file)
                continue
            if relpath in ["EPUB/package.opf", "EPUB/nav.xhtml", "EPUB/nav.ncx"]:
                # skip package.opf, nav.xhtml and nav.ncx (we are building updated versions of them, and discard nav.ncx)
                continue
    ...
    with create_single_html(epub_dir, spine, title, authors, fix_heading_levels, add_header_element) as single_html_obj:
        shutil.copy(single_html_obj.name, os.path.join(
            target_dir, f"EPUB/{identifier}.xhtml"))
    ...
    with create_updated_navigation_document(epub_dir, spine, identifier) as updated_nav_obj:
        shutil.copy(updated_nav_obj.name, os.path.join(
            target_dir, "EPUB/nav.xhtml"))

    properties = get_content_file_properties(
        os.path.join(target_dir, f"EPUB/{identifier}.xhtml"))
    with create_updated_package_document(epub_dir, spine, identifier, properties) as updated_opf_obj:
        shutil.copy(updated_opf_obj.name, os.path.join(
            target_dir, "EPUB/package.opf"))
    ...
    # result_as_file = result_as_file_obj if isinstance(
    #    result_as_file_obj, str) else result_as_file_obj.name
...
def create_single_html(epub_dir: str, spine: List[Dict[str, Union[str, int]]], title: str, authors: List[str], fix_heading_levels: bool, add_header_element: bool) -> tempfile._TemporaryFileWrapper:
    ...
    with open(single_html_obj.name, 'w', encoding="utf-8") as single_html_file:
        ...
        if add_header_element:
            single_html_file.write("\n<header>\n")
            single_html_file.write(
                f'    <h1 epub:type="fulltitle" class="title">{title}</h1>\n')
            for author in authors:
                single_html_file.write(
                    f'    <p epub:type="z3998:author" class="docauthor">{author}</p>\n')
        ...
        for i, item in enumerate(spine):
            ...
            next_root_level = int(
                spine[i + 1]["root_level"]) if i < len(spine) - 1 else int(spine[0]["root_level"])
            ...
            for content_type, text in iterate_content_file(path):
                if content_type == ContentType.BODY_TAG:
                    ...
                    if not depth.isnumeric():
                        logging.error(
                            f"Could not parse heading level from '{text}'")
                    else:
                        depth = str(
                            max(1, min(6, int(depth) + root_level - 1)))
                    single_html_file.write(
                        f"<{'/' if end else ''}h{depth}{rest}")
                    ...
                    if reference in [spine_item["href"] for spine_item in spine]:
                        ...
                        if "#" in value:
                            ...
                            logging.warning(
                                f"Reference to spine item '{reference}' without anchor found in '{path}': '{text}'")
                            value = ""
...
def create_updated_navigation_document(epub_dir: str, spine: List[Dict[str, Union[str, int]]], identifier: str) -> tempfile._TemporaryFileWrapper:
    nav_path = os.path.join(
        epub_dir, epub.get_nav_path(epub_dir) or "EPUB/nav.xhtml")
...
def create_updated_package_document(epub_dir: str, spine: List[Dict[str, Union[str, int]]], identifier: str, properties: List[str]) -> tempfile._TemporaryFileWrapper:
    opf_path = os.path.join(epub_dir, epub.get_opf_path(
        epub_dir) or "EPUB/package.opf")
            ...
            if tag.startswith("<item "):
                ...
                if item_href == "EPUB/nav.ncx":
                    ...
                    if not spine_ids:
                        ...
                        opf_file.write(
                            f'<item id="{item_id}" media-type="application/xhtml+xml" href="{identifier}.xhtml"')
                        if properties:
                            opf_file.write(' properties="' +
                                           " ".join(properties) + '"')
                        opf_file.write('/>\n')
                ...
                opf_file.write(indentation_text *
                               indentation_depth + "<spine>\n")
                opf_file.write(indentation_text * (indentation_depth + 1))
                opf_file.write(
                    f'<itemref idref="{spine_ids[0]}" id="itemref"/>\n')
                opf_file.write(indentation_text *
                               indentation_depth + "</spine>\n")
                indentation_depth -= 1
